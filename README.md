...
npm run questions -- meguru --set 4 "適正な査定額を出してくれる会社の見分け方"   # 4 問目だけ差し替え
```
...
1 問だけ直したいときは `--set <番号> "<質問文>"` を使います（番号はそのまま、地域名入りかどうかは本文から判定し直します）。差し替えは次回の `scan` から効き、過去の計測結果は当時の質問文のまま表示されます。「査定を高めに出してくれる会社」のように客が損をしかねない聞き方は、誠実な事業者が答えとして選ばれず計測の意味がなくなるので、「適正な査定額を出してくれる会社の見分け方」のように客の本当の関心に沿った聞き方に直してください（生成プロンプトにも同じ指示を入れています）。
