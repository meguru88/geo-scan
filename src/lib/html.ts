...
function scoreRow(s: EngineScore | (Aggregate['overall'] & { label: string; model?: string; manual?: boolean }), strong = false): string {
  const tag = strong ? 'th' : 'td';
  return `<tr class="${strong ? 'row-total' : ''}">
    <${tag} scope="row">${h(s.label)}${'manual' in s && s.manual ? ' <span class="pill">手動</span>' : ''}</${tag}>
    <td class="muted small">${h(s.model ?? '')}</td>
    <td class="num"><strong>${s.total}</strong>${bar(s.total, 100)}</td>
...
const CSS = `
:root { --accent: #1f4a2e; --accent-soft: #e8efea; --ink: #1c1c1c; --muted: #6b6b6b; --line: #dcdcdc; --warn: #b8860b; --bad: #b03a2e; }
...
.banner { background: #fff4d6; border: 1px solid #e6c56b; padding: 8px 12px; border-radius: 6px; font-size: 13px; margin-top: 14px; }
...
.qtable td.q { min-width: 230px; }
.qtable th.mark, .qtable td.mark { min-width: 64px; }
.qtable td.mark { text-align: center; font-size: 18px; line-height: 1.1; white-space: nowrap; }
.qtable td.mark .runs { display: block; font-size: 10px; color: var(--muted); }
.mark-o { color: var(--accent); font-weight: 800; }
.mark-d { color: var(--warn); font-weight: 800; }
.mark-x { color: var(--bad); }
.mark-n { color: #aaa; }
...
<header class="cover">
  ...
  <dl class="meta">
    ...
    <dt>計測日</dt><dd>${h(agg.date)}${agg.meta?.startedAt ? `（${fmtDateTime(agg.meta.startedAt)}〜${fmtDateTime(agg.meta.finishedAt)}）` : ''}</dd>
    <dt>計測内容</dt><dd>${agg.questions.length} 問 × ${agg.engines.length} エンジン${runs > 1 ? ` × ${runs} 回` : ''}（有効回答 ${o.answers} 件）</dd>
  </dl>
  <div class="score-label">総合スコア</div>
  <div class="score-hero"><div class="score-big">${Math.round(o.total)}</div><div class="score-denom">/ 100</div></div>
  <div class="breakdown">
  ...
  <p class="muted small">${h(scoreLabel(o.total))}。スコアは「AI の回答に社名が出た割合」「出た時の順番」「自社サイトが根拠として引用された割合」から算出しています。</p>
...
<section>
  ...
  <div class="table-wrap"><table>
    <thead><tr><th>エンジン</th><th>モデル</th><th class="num">スコア</th><th class="num">言及率</th><th class="num">平均順位</th><th class="num">自社サイト引用率</th><th class="num">有効回答</th></tr></thead>
    <tbody>
      ${autoEngines.map((e) => scoreRow(e)).join('')}
      ${manualEngines.map((e) => scoreRow(e)).join('')}
      ${scoreRow({ ...o, label: '総合', model: '' }, true)}
    </tbody>
  </table></div>
  <p class="legend">言及率 = 社名（別名含む）が回答に出た割合。平均順位 = 出た時に何番目の業者として挙がったか。自社サイト引用率 = 回答の出典に ${h(t.url.replace(/^https?:\/\//, ''))} が含まれた割合。</p>
</section>`;
...
<section>
  ...
  <div class="table-wrap"><table class="qtable">
    <thead><tr><th>#</th><th>質問</th>${agg.engines.map((e) => `<th class="mark" style="text-align:center">${h(agg.byEngine.find((x) => x.engine === e)?.label ?? e)}</th>`).join('')}</tr></thead>
    <tbody>
      ${agg.questionRows
        .map(
          (q) => `<tr><td class="num">${q.no}</td><td class="q">${h(q.text)}</td>${agg.engines
            .map((e) => {
              const c = q.cells[e];
              if (!c) return '<td class="mark mark-n">－</td>';
              const ranks = c.ranks.length ? `順位 ${c.ranks.join('/')}` : '';
              const title = c.okRuns ? `${c.mentionedRuns}/${c.okRuns} 回で言及${ranks ? '・' + ranks : ''}${c.citedRuns ? `・自社引用 ${c.citedRuns} 回` : ''}` : '有効な回答なし';
              return `<td class="mark ${markClass(c.mark)}" title="${h(title)}">${c.mark}<span class="runs">${c.okRuns ? `${c.mentionedRuns}/${c.okRuns}${ranks ? ' ' + ranks.replace('順位 ', '#') : ''}` : ''}</span></td>`;
            })
            .join('')}</tr>`,
        )
        .join('')}
  ...
  <p class="legend">○ = 全回で社名が出た　△ = 一部の回で出た（例: 3 回中 1〜2 回）　× = 出なかった　－ = 有効な回答なし。数字は「言及回数/有効回答数」と、出た時の順位（#）。</p>
</section>`;
...
<section>
  ...
  ${
    top5.length
      ? `<div class="table-wrap"><table>
    <thead><tr><th>#</th><th>業者</th><th class="num">言及回数</th><th>なぜ選ばれたか（回答文からの抜粋）</th></tr></thead>
    <tbody>${top5
      .map(
        (c, i) => `<tr><td class="num">${i + 1}</td><th scope="row">${h(c.name)}${c.isKnownCompetitor ? '' : ' <span class="pill">新出</span>'}</th><td class="num">${c.mentions} <span class="muted small">/ ${o.answers}</span></td><td>${h(c.reasons[0] ?? '（理由の記載なし）')}</td></tr>`,
      )
      .join('')}</tbody>
  </table></div>
  <p class="legend">言及回数は有効回答 ${o.answers} 件のうち、その業者名が出た回答の数。「新出」は設定ファイルの競合リストにない業者。</p>`
...
<section>
  ...
  ${
    ...
    <tbody>${top10
      .map(
        (d, i) =>
          `<tr class="${d.isOwn ? 'own' : ''}"><td class="num">${i + 1}</td><td>${h(d.domain)}${d.isOwn ? ' <span class="pill">自社</span>' : ''}</td><td class="num">${d.count} <span class="muted small">/ ${o.answers}</span></td><td class="small muted">${Object.entries(d.byEngine)
            .map(([e, n]) => `${h(agg.byEngine.find((x) => x.engine === e)?.label ?? e)} ${n}`)
            .join('、')}</td></tr>`,
      )
      .join('')}</tbody>
  </table></div>
...
<section>
  ...
  const modelList = agg.engines
    .map((e) => `${agg.byEngine.find((x) => x.engine === e)?.label ?? e}: ${agg.models[e] ?? '-'}`)
    .join(' / ');
  const methodology = `
<section>
  <h2>6. 計測方法と注意</h2>
  <ul class="method">
    <li>計測日時: ${agg.meta?.startedAt ? `${fmtDateTime(agg.meta.startedAt)} 〜 ${fmtDateTime(agg.meta.finishedAt)}` : h(agg.date)}（レポート作成 ${fmtDateTime(agg.generatedAt)}）</li>
    <li>各 AI に同じ ${agg.questions.length} 問を${runs > 1 ? ` ${runs} 回ずつ` : ''}質問し、Web 検索（最新情報の参照）を有効にした状態で回答を取得。指示文は「日本語で、具体的な業者名を挙げて答えてください」のみで、特定の業者を誘導していません。</li>
    <li>使用モデル: ${h(modelList)}</li>
    ...
    <li>社名の判定は表記ゆれ（${h(t.aliases.join('・'))}）を含めて行い、業者名と理由の抽出には AI（${h(agg.meta ? 'Claude' : '手動')}）を併用しています。</li>
    ${agg.totals.errors ? `<li>API エラー等で回答を取得できなかった ${agg.totals.errors} 件は集計から除外しています。</li>` : ''}
    ${agg.totals.manual ? `<li>${h(manualEngines.map((e) => e.label).join('・'))} は API がないため、実際の画面を目視で確認した結果（${agg.totals.manual} 件）を手入力で取り込んでいます。</li>` : ''}
