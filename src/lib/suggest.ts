...
/** 改善提案の候補（Claude にもテンプレートにも渡す） */
export const CANDIDATES: { key: string; title: string; why: string; action: string }[] = [
  {
    key: 'price',
    title: '日付つきの買取価格・実績を公開する',
    why: 'AI は「いつの情報か」が明記された具体的な数字（価格・件数）を根拠として引用しやすい。',
    action: '「2026年9月の金買取価格（g単価）」「今月の出張買取 件数」のように日付入りで毎月更新するページを作る。',
  },
  {
    key: 'area',
    title: '地域名を含むページを作る',
    why: '「東住吉区」「大阪市」など地域名入りの質問に対して、地域名が本文・見出しに入ったページが根拠として選ばれやすい。',
    action: '地域×品目（例:「東住吉区の貴金属出張買取」）のページを作り、対応エリア・出張の流れ・実例を載せる。',
  },
  {
    key: 'nap',
  ...
  },
  {
    ...
    why: '「出張費は無料？」「対応エリアは？」といった質問形式の情報は AI の回答にそのまま使われやすい。',
  ...
  },
  {
    ...
    action: '地域情報サイト・買取比較サイト・商工会などへの掲載を依頼し、取材記事やプレスリリースも活用する。',
  },
  {
    key: 'llms',
    title: 'llms.txt を設置する',
    why: 'AI クローラー向けに会社概要・サービス・対応地域・連絡先を簡潔にまとめたファイルがあると、要点が正しく拾われやすい。',
    action: 'サイト直下に /llms.txt を置き、社名・所在地・対応エリア・取扱品目・料金の考え方・問い合わせ先を記述する。',
  },
];
...
export function templateAdvice(agg: Aggregate): Advice {
  ...
  const suggestions: Suggestion[] = chosen.map((c) => {
    let why = c.why;
    if (c.key === 'area' || c.key === 'faq') why += ` 今回の自社サイト引用率は ${pct(o.citeRate)}（${o.answers} 回答中 ${o.cited} 回）でした。`;
    if (c.key === 'third' || c.key === 'gbp') why += ` 今回の言及率は ${pct(o.mentionRate)}（${o.answers} 回答中 ${o.mentioned} 回）でした。`;
    return { title: c.title, why, action: c.action };
  });
...
export async function claudeAdvice(agg: Aggregate): Promise<Advice> {
  ...
  const user = [
    ...
    '',
    '候補となる施策:',
    ...CANDIDATES.map((c) => `- ${c.title}: ${c.why}`),
    '',
    '出力:',
